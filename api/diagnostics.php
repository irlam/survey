...
if (!empty($_GET['probe']) && $_GET['probe'] === 'dwg') {
  ...
  $cands = ['dwg2pdf','dwg2svg','dwg2dxf','pdf2svg','convert','magick','pdfimages','ODAFileConverter','TeighaFileConverter','libredwg'];
  $found = [];
  foreach ($cands as $c){ $p = cmd_exists_probe($c); if ($p) $found[] = $c; }
  $hasMagick = (bool)cmd_exists_probe('magick') || (bool)cmd_exists_probe('convert');
  $info['dwg'] = ['found' => $found, 'imagemagick' => $hasMagick];
  json_response($info);
}
