  ...
  (async ()=>{
    ...
    (async ()=>{
      try{
        ...
        // place preview pin overlay inside previewWrap
        function placePreviewPin(){ try{ const existing = previewWrap.querySelector('.preview-pin'); if(existing) existing.remove(); const w = previewWrap.clientWidth || previewCanvas.clientWidth || previewCanvas.width || 0; const h = previewWrap.clientHeight || previewCanvas.clientHeight || previewCanvas.height || 0; if(!w||!h) return; const el = document.createElement('div'); el.className='pin preview-pin'; el.title = pin.title || ''; const left = (pin.x_norm !== undefined ? (pin.x_norm * w) : (0.5 * w)); const top = (pin.y_norm !== undefined ? (pin.y_norm * h) : (0.5 * h)); el.style.left = left + 'px'; el.style.top = top + 'px'; const labelText = String(pin.id || pin.label || pin.title || '!'); const fontSize = labelText.length <= 2 ? 12 : (labelText.length === 3 ? 10 : 9); if(_pinSvgText){ try{ const parser = new DOMParser(); const doc = parser.parseFromString(_pinSvgText, 'image/svg+xml'); const svgEl = doc.querySelector('svg'); if(svgEl){ const node = document.importNode(svgEl, true); const txt = node.querySelector('.pin-number'); if(txt){ txt.textContent = labelText; txt.setAttribute('font-size', String(fontSize)); } el.appendChild(node); } else { el.innerHTML = `<svg viewBox="0 0 64 80" width="56" height="72"><circle cx="32" cy="20" r="16" fill="#e12b2b"/><path d="M32 36 C24 48, 16 60, 32 76 C48 60, 40 48, 32 36 Z" fill="#e12b2b"/><text x="32" y="20" text-anchor="middle" dominant-baseline="central" fill="#000" font-weight="900" font-size="${fontSize}">${labelText}</text></svg>`; } }catch(e){ console.warn('preview pin SVG parse failed', e); el.innerHTML = `<svg viewBox="0 0 64 80" width="56" height="72"><circle cx="32" cy="20" r="16" fill="#e12b2b"/><path d="M32 36 C24 48, 16 60, 32 76 C48 60, 40 48, 32 36 Z" fill="#e12b2b"/><text x="32" y="20" text-anchor="middle" dominant-baseline="central" fill="#000" font-weight="900" font-size="${fontSize}">${labelText}</text></svg>`; } previewWrap.appendChild(el); }catch(e){ console.warn('placePreviewPin failed', e); } }
        const ensurePreview = () => {
