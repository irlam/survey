...
function showIssuesModal(planId) {
  ...
  closeBtn.onclick = () => { modal.style.display = 'none'; document.removeEventListener('photosUpdated', photosListener); };
  window.onclick = (event) => { if (event.target === modal) { modal.style.display = 'none'; document.removeEventListener('photosUpdated', photosListener); } };
